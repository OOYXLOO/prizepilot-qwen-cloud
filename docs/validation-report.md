...
- `$env:PYTHONPATH='src'; python -m prizepilot cloud-readiness --checked-at 2026-06-15T12:30:00Z` is covered by tests as a deterministic evidence rebuild path for review diffs; it does not replace the fresh action-time cloud readiness run before a public update.
