# Demo Video Upload Pack

Use this when a YouTube, Vimeo, or Facebook Video upload session is available. The Devpost video field also displays Youku as a generic accepted host, but the Qwen official requirements say videos must be uploaded to YouTube, Vimeo, or Facebook Video and made public, so prefer those three for rule compliance.

## Local File

- `docs/demo-video/prizepilot-demo.webm`

## Upload Settings

- Visibility: public or unlisted/publicly viewable, as long as Devpost can access it without login.
- License/category: standard/default.
- Audience: not made for children.
- Comments: optional.
- Captions: none needed; the video is a short silent product demo.

## Title

PrizePilot Qwen Cloud Hackathon Demo

## Short Description

PrizePilot is a Qwen-ready Track 4 Autopilot Agent for turning hackathon and bounty prize pages into ranked execution plans, public artifact drafts, and human approval checkpoints.

The demo shows a cash-prize portfolio ranking, the Qwen Blog Post Award / Honorable Mention route, the local dashboard, and the integrity boundary for API keys, deployment proof, public posts, and Devpost evidence updates.

Public repository:
https://github.com/OOYXLOO/prizepilot-qwen-cloud

Build journal:
https://ooyxloo.github.io/prizepilot-qwen-cloud/blog/

## Tags

Qwen Cloud, hackathon, AI agent, autopilot agent, Devpost, Alibaba Cloud, PrizePilot

## Devpost Video Field

Accepted public Vimeo URL:

```text
https://vimeo.com/1200124146
```

The same URL is recorded in `docs/qwen-route-ledger.md` under `Demo video URL` and accepted by the submitted Devpost project.

## Do Not Claim

Do not claim live Qwen API usage from the video alone; cite the separate Qwen live proof page. Do not claim Alibaba Cloud deployment until separate endpoint evidence exists.
