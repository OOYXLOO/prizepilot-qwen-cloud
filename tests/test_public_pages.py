...
class PublicPagesTests(unittest.TestCase):
    def test_judge_pack_is_linked_from_public_hub(self) -> None:
        ...
        self.assertIn("./final-judge-one-page/", hub)
        ...
        self.assertIn("../final-judge-one-page/", judge_pack)
        ...
        self.assertTrue((ROOT / "docs" / "final-judge-one-page" / "index.html").is_file())
        self.assertTrue((ROOT / "docs" / "final-judge-one-page.md").is_file())
    ...
    def test_readme_exposes_judge_pack_url(self) -> None:
        ...
        self.assertIn("https://ooyxloo.github.io/prizepilot-qwen-cloud/final-judge-one-page/", readme)
        ...
        self.assertIn("New review links that must be HTTP 200 rechecked after the repository update", readme)
    ...
    def test_public_update_digest_explains_local_public_boundary(self) -> None:
        ...
        self.assertIn("Last updated: 2026-06-26 (+08)", markdown)
        self.assertNotIn("Last updated: 2026-06-15 (+08)", markdown)
        self.assertNotIn("Last updated: 2026-06-14", markdown)
        self.assertIn("Previous public repository baseline", markdown)
        self.assertIn("Repository update head", markdown)
        ...
        self.assertIn("Only after the account owner is present and the public URLs are rechecked", markdown)
        self.assertIn("PrizePilot's repository update packet is ready for public link checks.", page)

    def test_final_judge_one_page_summarizes_award_case_and_boundaries(self) -> None:
        markdown = (ROOT / "docs" / "final-judge-one-page.md").read_text(encoding="utf-8")
        page = (ROOT / "docs" / "final-judge-one-page" / "index.html").read_text(encoding="utf-8")

        self.assertIn("PrizePilot Final Judge One-Page", markdown)
        self.assertIn("Track 4 Autopilot Agent", markdown)
        self.assertIn("Qwen/DashScope live smoke proof", markdown)
        self.assertIn("Blog Post Award", markdown)
        self.assertIn("Top 10 Honorable Mention Projects", markdown)
        self.assertIn("Reviewer fast path", markdown)
        self.assertIn("Do not infer prize selection", markdown)
        self.assertIn("Do not claim a live Alibaba Cloud public endpoint exists", markdown)
        self.assertIn("Final judge one-page", page)
        self.assertIn("Track 4 Autopilot Agent", page)
        self.assertIn("Qwen evidence", page)
        self.assertIn("still pending", page)
        self.assertIn("../judge-pack/", page)
        ...
        self.assertIn("repository update packet contains prepared review commits after that baseline", page)
        ...
        self.assertIn("HTTP 200 after the repository update", page)
    ...
    def test_publication_action_card_separates_live_baseline_from_devpost_gate(self) -> None:
        card = (ROOT / "docs" / "publication-action-card.md").read_text(encoding="utf-8")

        self.assertIn("Current state on 2026-06-26", card)
        ...
        self.assertIn("## Repository Update Packet", card)
        self.assertIn("Public baseline links currently live", card)
        self.assertIn("repository update packet and must be HTTP 200 rechecked before any Devpost copy references them", card)
