...
class WebappTests(unittest.TestCase):
    def test_dashboard_payload_exposes_qwen_and_portfolio(self) -> None:
        ...
        self.assertEqual(len(payload["qwen_agent_flow"]), 5)
        self.assertEqual(payload["qwen_agent_flow"][0]["stage"], "1. Public prize page")
        self.assertIn("qwen-plus", payload["qwen_proof_snapshot"]["verified"])
        self.assertIn("HTTP 200 at / and /api/plan", payload["qwen_proof_snapshot"]["still_pending"])
        self.assertEqual(len(payload["agent_walkthrough"]), 5)
        self.assertEqual(len(payload["judge_scorecard"]), 4)
        self.assertTrue(any(gap.startswith("Qwen/DashScope live refinement is verified") for gap in payload["evidence_gaps"]))
    ...
    def test_dashboard_html_links_json_endpoint_and_shows_verified_submission(self) -> None:
        ...
        self.assertIn("https://ooyxloo.github.io/prizepilot-qwen-cloud/qwen-live-proof/", html)
        self.assertIn("Qwen Agent Flow", html)
        self.assertIn("Qwen proof snapshot", html)
        self.assertIn("Public proof packet", html)
        self.assertIn("live qwen-plus smoke receipt", html)
